...
\begin{enumerate}
    ...
    \item \textcolor{azulito}{Energía potencial dentro de sus partes}
\end{enumerate}
...
\begin{center}
	\begin{tikzpicture}
		...
		% ~~~~ Flechas ~~~~ %
		\draw[ -stealth,amarillito ] (-2.85355339,0.6464466) -- (-3.5,0)
		node [midway, yshift=-4,xshift=8] {$v_1$};
		\draw[ -stealth, amarillito ] (-1.25,2.366987) -- (-1.75,1.50096189)
		node [midway, yshift=0,xshift=8] {$v_2$};
		\draw[ -stealth, amarillito ] (1.85355339,2.14655661) -- (3,1)
		node [midway, yshift=-2,xshift=-8] {$v_3$};

        \draw[stealth-stealth, azulito] (0.512132034,-2.28786797) -- (1.78786797,-1.01213203)
            node[midway,yshift=-4,xshift=4] {$F$};
        \draw[stealth-stealth,azulito] 
        ({0.5*cos(deg(0.3*pi))-2.5},{0.5*sin(deg(0.3*pi))+1}) 
            -- ({0.5*cos(deg(1.2*pi))-1},{0.5*sin(deg(1.2*pi))+2.8})
            node [midway,xshift=-3,yshift=3] {$F$};
        \draw[stealth-stealth, azulito] ({0.5*cos(deg(0*pi))-1},{0.5*sin(deg(0*pi))+2.8})
            -- ({0.5*cos(deg(pi))+1.5},{0.5*sin(deg(pi))+2.5})
            node [midway, above] {$F$};
	\end{tikzpicture}
\end{center}
...
\begin{center}
	\begin{tikzpicture}
		...
		node {$B\,,\,T_B$};
		% ~~~~ Rayos/Rep.Energia ~~~~ %
		\draw[amarillito, -stealth] (1, 0.5) -- (2, 0.6) -- (1.8, 0.4) -- (3, 0.5)
		node[midway,xshift=-12.5, yshift=10] {Energía};
		\draw[amarillito, -stealth] (2.9,-0.1) -- (1.8,-0.2) -- (2,0) -- (1.1,-0.1);
	\end{tikzpicture}
\end{center}
